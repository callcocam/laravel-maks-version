...
class OrderAviament extends Model
{
    ...
    {
        return $this->hasMany(ItemAviament::class)->where($where);
    }

    protected function posSave($data)
    {

        if (isset($data['aviament'])) {
            if ($data['aviament']) {
                $aviament = $data['aviament'];
                if (!empty($aviament["parent"])) {
                    $this->addItem($data, $aviament["parent"],[
                        "order_id" => $data["id"],
                        "aviament_id" => $aviament["parent"]
                    ]);
                }
            }
        }
    ...
    protected function addItem($data, $parent, $ruery){
        ...
        $dataItem["assets"] = 'aviaments';
        $dataItem["aviament_id"] = $parent;
        $dataItem["amount"] = 1;
